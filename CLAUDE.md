...
| PRES-023 | 2026-08-22 | **El deck NO se adapta a móvil.** Se compone exclusivamente para **16:9 horizontal**, porque el entregable es PowerPoint 1920×1080 + video. Ver en celular es un caso de revisión, no de entrega: se mira girando el equipo. No invertir esfuerzo en layouts responsivos verticales ni sacrificar composición por pantallas angostas. |

| PRES-024 | 2026-08-22 | **Deck a 14 slides: nueva 06 ANATOMÍA DE LAS PARTES** (entre El módulo y El montaje). Tres piezas IFC **a escala real** lado a lado: crujía suelo (placeholder procedural hasta recibir IFC) · **crujía tipo** (`crujia_tipo.ifc`, 134 elem, 121k tri) · **crujía uniones** (`crujia_uniones.ifc`, 46 elem, 63k tri). Tratamiento **híbrido sólido sombreado + contorno fino**; Danpal translúcido. **Zoom al tocar**: la pieza se amplía y centra, las otras se atenúan, arrastrar rota; rótulos de familia (vigas, arriostramiento, nudos, subestructura, Danpal, perfiles) aparecen al ampliar. Geometría completa sin decimar: 1,8 ms/frame. Horneado: vértices soldados a 0,5 mm + int16 mm (4,3→2,1 MB). Deck 3,75 MB. |
...
| PRES-028 | 2026-08-23 | **FASE 2 - nueva estructura de trabajo en equipo** en Dropbox `PAC/04_PRODUCTOS/`: carpeta por slide (01-16) para fuentes y productos, `00_DECK/` con la **fuente unica** (`PAC7.0-WSEF_deck_v30.html`, la version entregada en F1) y **protocolo de lock cooperativo** via `ESTADO_DECK.md` (LIBRE/EN_EDICION + bitacora de versiones v31, v32...; toda IA lo lee antes de editar). `90_PIPELINE/` con scripts + `CONTEXTO_IA.md` (manual autocontenido para cualquier IA del equipo, absorbe el encargo ARCHIVOS RSZ), `91_MODELOS_IFC/` con los 5 IFC. Se creo `06_ENTREGA F2/` vacia. Los originales de F1 en `05_ENTREGA F1/` no se tocan. |
